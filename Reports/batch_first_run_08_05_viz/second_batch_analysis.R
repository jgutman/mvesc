...
# set wd
setwd('~/mvesc/Reports/batch_second_run_0810/')
...
batch_two = reports_ref %>% filter(cv_criterion == 'custom_precision_10',
                                 debug == FALSE,
                                 timestamp > '2016-08-09 06:00:00')
...
compare_cv_scheme = function(ref, score_col){
  ...
  top_ref = grab_top_performing(ref, "cv_score", 10,
  ...
  plot_df = df %>% ungroup() %>% gather(recall_k, recall, val_recall_3:test_recall_10) %>%
    separate(recall_k, c("dataset", "metric_type", "k_pcnt_intervene"), sep = "_") %>%
    group_by(model_name, prediction_grade, metric_type, k_pcnt_intervene,
             cv_scheme) %>% 
    ...
    filter(prediction_grade %in% c(7, 8)) %>%
  ...
  g <- ggplot(plot_df, aes(x = k_pcnt_intervene,
                           y = avg_recall,
                           color = cv_scheme,
                           group = cv_scheme_time))
  
  g + geom_point() + facet_grid(prediction_grade ~ model_name) + geom_line()
}
...
gather_recall_3_10 = function(df, vec_to_group_on){
  df %>% ungroup() %>% gather(recall_k, recall, val_recall_3:test_recall_10) %>%
    separate(recall_k, c("dataset", "metric_type", "k_pcnt_intervene"), sep = "_") %>%
