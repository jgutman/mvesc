...
with_cutoff_line = with_cutoff_line %>% 
  mutate(cutoff_val = ifelse(cutoff == TRUE, last(predicted_score), 0))

ggplot(with_cutoff_line, aes(x = predicted_score, fill = factor(true_label),
                          color = factor(true_label))) + 
  facet_grid(model_name~split) +
  geom_freqpoly() + ylim(0, 50) + xlim(0.01, 1) + geom_vline(aes(xintercept = cutoff_val))


#### LOOKING AT HIGH DIFFERENCE STUDENTS ####

divergent_students = by_id_diff %>% filter(split != 'train') %>%
